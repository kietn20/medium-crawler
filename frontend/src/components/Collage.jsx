...
export const Collage = () => {
	...
	const dockElements = [
		...
		},
		{
			title: "Share Collage",
			icon: (
				<Share className="h-full w-full dark:text-neutral-3001 text-[#8ac847]" />
			),
			href: "#",
		},
		{
			...
			<div className="mt-3 w-[822px] h-[614px] grid grid-cols-4 gap-x-[20px] gap-y-[30px] items-center place-items-center place-content-between">
				{mediaItems.map((mediaItem, index) => (
					<div key={index} className="relative">
						{showRanking && (
							<div className="absolute text-3xl text-[#444C48] -bottom-7 -left-7 w-[50px] h-[50px] bg-[#B1FA63] bg-opacity-80 font-heading text-center rounded-lg flex justify-center items-end z-10">
								&nbsp;{index + 1}.
							</div>
						)}
						<Slot index={index} />
